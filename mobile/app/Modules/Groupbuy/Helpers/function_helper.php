 ...
 * 2018.8.6
 */
function paipai_buy_list($size, $page, $keywords, $sort, $order)
{
	$gb_list = array();
...
function paipai_buy_add_list($size, $page, $keywords, $sort, $order)
{
	...
	if ($sort == 'comments_number') {
		$sql = 'SELECT b.*, IFNULL(g.goods_thumb, \'\') AS goods_thumb, b.ppj_id AS group_buy_id, g.market_price,' . 'b.start_time AS start_date, b.end_time AS end_date ' . 'FROM ' . $GLOBALS['ecs']->table('paipai_list') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON b.goods_id = g.goods_id ' . 'WHERE b.act_type = \'' . GAT_PAIPAI_BUY . ('\' ' . $where . ' ') . (' AND b.start_time >= \'' . $now . '\' AND b.start_time <= \'' . $limit_time . '\'  AND b.ppj_staus < 3 AND b.review_status = 3 ORDER BY g.') . $sort . ' ' . $order;
	}
	else {
		$sql = 'SELECT b.*, IFNULL(g.goods_thumb, \'\') AS goods_thumb, b.ppj_id AS group_buy_id, g.market_price,' . 'b.start_time AS start_date, b.end_time AS end_date ' . 'FROM ' . $GLOBALS['ecs']->table('paipai_list') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON b.goods_id = g.goods_id ' . 'WHERE b.act_type = \'' . GAT_PAIPAI_BUY . ('\' ' . $where . ' ') . (' AND b.start_time >= \'' . $now . '\' AND b.start_time <= \'' . $limit_time . '\' AND b.ppj_staus < 3 AND b.review_status = 3 ORDER BY b.') . $sort . ' ' . $order;
	}
//	$res = $GLOBALS['db']->selectLimit($sql, $size, ($page - 1) * $size);
...
function get_filter_one($id)
{
	...
	$sql = 'SELECT * FROM {pre}paipai_list WHERE review_status = 3 and ppj_id = ' . $id;
...
{
	$ru_id = $GLOBALS['db']->getOne('SELECT user_id FROM ' . $GLOBALS['ecs']->table('paipai_list') . (' WHERE ppj_id = \'' . $group_buy_id . '\' AND review_status = 3'));
	$sql = 'SELECT ga.ppj_id, ga.ext_info, ga.ppj_name, g.goods_thumb, g.sales_volume FROM ' . $GLOBALS['ecs']->table('paipai_list') . ' ga' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' g ON ga.goods_id = g.goods_id ' . (' WHERE ga.user_id = \'' . $ru_id . '\' AND act_type = \'') . GAT_PAIPAI_BUY . '\' AND ga.review_status = 3 LIMIT 4 ';
...
{
	$sql = 'SELECT ga.*, g.sales_volume, g.goods_thumb, g.goods_id FROM ' . $GLOBALS['ecs']->table('paipai_list') . ' ga' . ' LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' g ON ga.goods_id = g.goods_id ' . (' WHERE ga.user_id = \'' . $user_id . '\' AND g.goods_id > 0 AND act_type = \'') . GAT_PAIPAI_BUY . '\' AND ga.review_status = 3 ORDER BY g.' . $order . ' LIMIT 5 ';
