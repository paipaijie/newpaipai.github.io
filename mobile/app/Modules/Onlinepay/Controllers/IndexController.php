 ...
 */
	...
	{
		$payment_id = I('pay_id', 0, 'intval');//17 微信支付
		...
		if (IS_AJAX) {
			$payment = payment_info($payment_id);  // app/Helpers/order_helper.php获取支付配置详细信息
			...
			//echo get_code($order, unserialize_config($payment['pay_config']));
			
			exit($pay_online); //exit() 函数输出一条消息，并退出当前脚本。
		}
