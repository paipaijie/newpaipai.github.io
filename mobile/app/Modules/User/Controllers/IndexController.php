...
public function actionMypaipaiList()
	{
		if (IS_AJAX) {
			$user_id = $this->user_id;
			...
			$sql='select count(*) from {pre}goods as g,{pre}paipai_list as pl where g.goods_id =pl.goods_id and  pl.goods_id in(select goods_id from {pre}paipai_seller where user_id=' . $user_id . ' and  usestaus=0 GROUP BY goods_id)';
...
public function actionMypaipaiokList()
	{
		...
		$this->assign('record_count', $collection_goods['record_count']);
		$this->assign('size', $collection_goods['size']);
		$this->assign('page_title', '成交的拍拍商品');
		$this->display();
	}
...
// 分享，邀请好友页面
	...
	public function actionAffiliate()
	{
		...
		$url = url('user/login/registershare', '', false, true) . '&u=' . $this->user_id; // 生成二维码的访问地址
		$uid=$this->user_id;
		...
		$this->assign('page_title', '推荐好友');
		$this->display();
	}

//我的推荐会员

public function actionAffiliatemeber()
	{
		$share = unserialize($GLOBALS['_CFG']['affiliate']);
		...
		if (empty($goodsid)) {
			...
			$sql = 'SELECT user_name FROM {pre}users WHERE parent_id IN(' . $up_uid . ')';
					
					$affdb = $GLOBALS['db']->query($sql);
	...
	public function actionTuijianren()
	{
		//echo "haha ";
		$url = 'public/img/0.jpg';
		$url1 = 'web/moblie/public/img/bj.jpg';
		$this->assign('url',$url);
