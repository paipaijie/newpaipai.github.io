...
///获取优惠券
	...
	// 获取用户的产品拍拍券数量和明细
	public function actionPaipaijuan()
	{
		$page = I('page', 1, 'intval');
		...
		if (IS_AJAX) {
			...
			$paipaijuan_list = get_paipaijuqn_lists($this->size, $page, $status);
			
			exit(json_encode(array('paipaijuan_list' => $paipaijuan_list, 'totalPage' => $paipaijuan_list['totalpage'])));
			
		}

		$this->assign('status', $status);
		$this->assign('page_title', '拍拍销售券');
		$this->display();
	}
	
 // 拍拍小店
 
	public function actionPaipaiindex()
	{
			
         $sql = "SELECT user_name,user_picture,nick_name FROM " . $GLOBALS['ecs']->table('users') .
