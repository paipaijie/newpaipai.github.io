...
function get_paipaijuqn_lists($num = 2, $page = 1, $status = 0)
{
	...
	if ($status == 0) {
		$where = 'where cu.usestaus = 0  and cu.user_id = \'' . $uid . '\' and cu.endtime>\'' . $time . '\' ';
	}
	else if ($status == 1) {
		$where = 'where cu.usestaus = 1  and cu.user_id = \'' . $uid . '\' ';
	}
	else if ($status == 2) {
		$where = 'where  \'' . $time . '\' > cu.endtime and  cu.usestaus = 2  and cu.user_id = \'' . $uid . '\'';
	}

	$sql = 'SELECT COUNT(*) FROM ' . $GLOBALS['ecs']->table('paipai_seller') . ' AS cu ' . $where ;
	...
	*/
	...
	foreach ($tab as &$v) {
		...
		$v['begintime'] = local_date('Y-m-d', $v['createtime']);
		
		$v['endtime'] = local_date('Y-m-d', $v['endtime']);
		...
		$v['img'] = 'images/coupons_default.png';
		  ...
		  if (!empty($v['goods_id'])) {
			...
			$v['goods_name'] = $GLOBALS['db']->getone('SELECT goods_name FROM ' . $GLOBALS['ecs']->table('goods') . ' WHERE goods_id = ' . $v['goods_id'] . '');
