...
/*by xul 
 *  
 * restrict_amount  1360至1484
 */
function paipai_buy_info($group_buy_id, $current_num = 0)
{
	$group_buy_id = intval($group_buy_id);
	
	//b.act_desc AS group_buy_desc,
	
	$sql = 'SELECT b.*,g.*, b.ppj_id AS group_buy_id, b.start_time AS start_date, b.end_time AS end_date ' . 'FROM ' . $GLOBALS['ecs']->table('paipai_list') . ' AS b ' . 'LEFT JOIN ' . $GLOBALS['ecs']->table('goods') . ' AS g ON b.goods_id = g.goods_id ' . ('WHERE ppj_id = \'' . $group_buy_id . '\' ') . 'AND act_type = \'' . GAT_PAIPAI_BUY . '\'' . 'AND b.review_status = 3';
 ...
 // 限购的数量
   $group_buy['restrict_amount'] =$group_buy['goods_count'] ;
	...
	$group_buy['formated_deposit'] = price_format($group_buy['ppj_margin_fee'], false);
	...
	$stat = paipai_buy_stat($group_buy_id,$group_buy['ppj_no'], $group_buy['ppj_margin_fee']);
	...
	$group_buy['formated_cur_price'] = price_format($cur_price, false);// 当前价格
 ...
 * $ppj_no 期数
 */
function paipai_buy_stat($group_buy_id, $ppj_no,$deposit)
{
	$group_buy_id = intval($group_buy_id);
	...
	if (0 < $deposit && 0 < $stat['total_order']) {
		...
		$stat['valid_order'] = $stat['total_order'];  // 订单总数
		$stat['valid_goods'] = $stat['total_goods'];  // 商品已售的数量
	}
