...
//我有权限的拍拍商品
function get_paipai_goods($user_id, $record_count, $limit = '')
{
	
	$sql='select g.*,pl.*  from {pre}goods as g,{pre}paipai_list as pl where g.goods_id =pl.goods_id and  pl.goods_id in(select goods_id from {pre}paipai_seller where user_id=' . $user_id . ' and  usestaus=0 GROUP BY goods_id)';
  ...
  $res = $GLOBALS['db']->getAll($sql);
	...
	foreach ($res as $key => $row) {
		...
		$goods_list[$row['goods_id']]['url'] = $row['goods_number'];
		$goods_list[$row['goods_id']]['goods_thumb'] = get_image_path($row['goods_thumb']);
		$goods_list[$row['goods_id']]['goods_name'] = $row['goods_name'];
			$goods_list[$row['goods_id']]['goods_name'] = $row['goods_name'];
	$goods_list[$row['goods_id']]['ppj_id'] = $row['ppj_id'];
	$goods_list[$row['goods_id']]['ppj_no'] = $row['ppj_no'];
	$goods_list[$row['goods_id']]['ppj_margin_fee'] = $row['ppj_margin_fee'];
	$goods_list[$row['goods_id']]['market_price'] = $row['market_price'];
	$goods_list[$row['goods_id']]['shop_price'] = $row['shop_price'];	
	$goods_list[$row['goods_id']]['del'] = $row['shop_price'];
	 ...
	 $ext_info = unserialize($row['ext_info']);
		...
		$nowprice = $row['price_ladder'][0]['price'];
		
		
		$stat = paipai_buy_stat($row['ppj_id'], $row['ppj_no'],$row['ppj_margin_fee']);//获取订单数
	...
	$row['cur_amount'] = $stat['valid_goods'];
		...
		$row['url'] = build_uri('groupbuy', array('gbid' => $val['ppj_id']));
				
		$row['cur_price'] = $cur_price;
	
	    $goods_list[$row['goods_id']]['url']=$row['url'];
		$goods_list[$row['goods_id']]['cur_amount']=$row['cur_amount'];
	   	$goods_list[$row['goods_id']]['price'] = price_format($cur_price, false);// 当前价格
...
function get_paipai_sellegoods($user_id, $record_count, $limit = '')
{
	
	$sql='select g.*,pl.*  from {pre}goods as g,{pre}paipai_seller_ok as pl where g.goods_id =pl.goods_id and pl.user_id=' . $user_id ;
  ...
  $res = $GLOBALS['db']->getAll($sql);
	...
	$goods_list[$row['goods_id']]['getmoney'] = price_format($row['getmoney']);
		
	$goods_list[$row['goods_id']]['goods_nowprice'] = price_format($row['goods_nowprice']);
